    ...
    public class GeneralConvexPairHandler : CollidablePairHandler
    {
        ...
        public override void UpdateTimeOfImpact(Collidable requester, float dt)
        {
                ...
                )
            {
                ...
                var minimumRadiusA = convexA.minimumRadius * MotionSettings.CoreShapeScaling;
                timeOfImpact = 1;
                if (minimumRadiusA * minimumRadiusA < velocitySquared)
                {
                    //Spherecast A against B.
                    RayHit rayHit;
                    if (GJKToolbox.SphereCast(new Ray(convexA.worldTransform.Position, -velocity), minimumRadiusA, convexB.Shape, ref convexB.worldTransform, 1, out rayHit))
                    {
                        timeOfImpact = rayHit.T;
                    }
                }

                var minimumRadiusB = convexB.minimumRadius * MotionSettings.CoreShapeScaling;
                if (minimumRadiusB * minimumRadiusB < velocitySquared)
                {
                    //Spherecast B against A.
                    RayHit rayHit;
                    if (GJKToolbox.SphereCast(new Ray(convexB.worldTransform.Position, velocity), minimumRadiusB, convexA.Shape, ref convexA.worldTransform, 1, out rayHit) &&
