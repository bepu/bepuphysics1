...
namespace BEPUphysics.NarrowPhaseSystems.Pairs
{
    ...
    ///</summary>
    public class CompoundInstancedMeshPairHandler : CollidablePairHandler, IPairHandlerParent
    {
        ...
        ///</summary>
        public CompoundInstancedMeshPairHandler()
        {
        ...
        public override void Initialize(BroadPhaseEntry entryA, BroadPhaseEntry entryB)
        {
            ...
            if (compoundInfo == null || mesh == null)
            {
            ...
            if (!suppressEvents)
            {
                compoundInfo.events.OnPairCreated(mesh, this);
                mesh.events.OnPairCreated(compoundInfo, this);
            }
        }
        ...
        public override void CleanUp()
        {
            ...
            if (shouldNotify)
            {
                compoundInfo.events.OnCollisionEnded(mesh, this);
                mesh.events.OnCollisionEnded(compoundInfo, this);
            }
            ...
            if (!suppressEvents)
            {
                compoundInfo.events.OnPairRemoved(mesh);
                mesh.events.OnPairRemoved(compoundInfo);
            }
        ...
        private void UpdatePairs()
        {
            ...
            {
                CollidablePair pair = new CollidablePair(overlappedElements.Elements[i].CollisionInformation, mesh);
                CollisionRule rule;
        ...
        public override void UpdateCollision(float dt)
        {
            ...
            if (!suppressEvents)
            {
                compoundInfo.events.OnPairUpdated(mesh, this);
                mesh.events.OnPairUpdated(compoundInfo, this);
            }
            ...
            if (contactCount > 0)
            {
                if (!suppressEvents)
                {
                    compoundInfo.events.OnPairTouching(mesh, this);
                    mesh.events.OnPairTouching(compoundInfo, this);
                }

                if (previousContactCount == 0)
                {
                    //collision started!
                    compoundInfo.events.OnInitialCollisionDetected(mesh, this);
                    mesh.events.OnInitialCollisionDetected(compoundInfo, this);
        ...
        void IPairHandlerParent.OnContactAdded(Contact contact)
        {
            ...
            if (!suppressEvents)
            {
                compoundInfo.events.OnContactCreated(mesh, this, contact);
                mesh.events.OnContactCreated(compoundInfo, this, contact);
            }
        }
        ...
        void IPairHandlerParent.OnContactRemoved(Contact contact)
        {
            ...
            if (!suppressEvents)
            {
                compoundInfo.events.OnContactRemoved(mesh, this, contact);
                mesh.events.OnContactRemoved(compoundInfo, this, contact);
            }
        }
        ...
        {
            timeOfImpact = 1;
            foreach (CollidablePairHandler pair in this.subPairs.Values)
            {
        ...
        internal override void GetContactInformation(int index, out ContactInformation info)
        {
            ...
            throw new IndexOutOfRangeException("Contact index is not present in the compound-instanced mesh pair.");
