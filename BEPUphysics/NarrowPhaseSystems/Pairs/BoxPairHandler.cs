    ...
    public class BoxPairHandler : CollidablePairHandler
    {
        ...
        public override void UpdateTimeOfImpact(Collidable requester, float dt)
        {
                ...
                )
            {
                ...
                var minimumRadiusA = boxA.minimumRadius * MotionSettings.CoreShapeScaling;
                timeOfImpact = 1;
                if (minimumRadiusA * minimumRadiusA < velocitySquared)
                {
                    //Spherecast A against B.
                    RayHit rayHit;
                    if (GJKToolbox.SphereCast(new Ray(boxA.worldTransform.Position, -velocity), minimumRadiusA, boxB.Shape, ref boxB.worldTransform, 1, out rayHit))
                    {
                        timeOfImpact = rayHit.T;
                    }
                }

                var minimumRadiusB = boxB.minimumRadius * MotionSettings.CoreShapeScaling;
                if (minimumRadiusB * minimumRadiusB < velocitySquared)
                {
                    //Spherecast B against A.
                    RayHit rayHit;
                    if (GJKToolbox.SphereCast(new Ray(boxB.worldTransform.Position, velocity), minimumRadiusB, boxA.Shape, ref boxA.worldTransform, 1, out rayHit) &&
