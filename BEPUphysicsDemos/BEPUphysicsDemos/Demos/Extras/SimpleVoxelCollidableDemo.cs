...
namespace BEPUphysicsDemos.Demos.Extras
{
    ...
    public class VoxelGridConvexContactManifold : ContactManifold
    {
        ...
        private QuickDictionary<Int3, GeneralConvexPairTester> activePairs;
        ...
        private GeneralConvexPairTester GetPair(ref Int3 position)
        {
            ...
            boxCollidable.WorldTransform = new RigidTransform(new Vector3(
                voxelGrid.Position.X + position.X * voxelGrid.Shape.CellWidth,
                voxelGrid.Position.Y + position.Y * voxelGrid.Shape.CellWidth,
                voxelGrid.Position.Z + position.Z * voxelGrid.Shape.CellWidth));
            return pair;
        }
            ...
            : base(game)
        {
            ...
            for (int i = 0; i < cellCountX; ++i)
            {
                for (int j = 0; j < cellCountY; ++j)
                {
                    ...
                    {
                        cells[i, j, k] = (Math.Sin(i * 0.55f + 6f + j * -0.325f) + Math.Sin(j * 0.35f - 0.5f + MathHelper.PiOver2) + Math.Sin(k * 0.5f + MathHelper.Pi + 7 + j * 0.25f)) > 0;
                    }
                }
            }
            var cellWidth = 1f;
                    ...
                    for (int k = 0; k < cellCountZ; ++k)
                    {
                        if (shape.Cells[i, j, k])
                        {
                            ...
                            game.ModelDrawer.Add(new DisplayModel(game.Content.Load<Model>("cube"), game.ModelDrawer)
                                {
                                    WorldTransform = Matrix.CreateWorldRH(grid.Position + new Vector3(i * cellWidth, j * cellWidth, k * cellWidth), Vector3.Forward, Vector3.Up)
                                });
                        }
                    }
                }
